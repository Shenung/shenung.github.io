...
func (info *financeInfo) parseTable(record []string) (*finance, error) {
	date, err := strconv.Atoi(record[info.columns["date"]])
	if err != nil {
		return nil, err
	}
	...
	shut := record[info.columns["shut"]]
	volume := record[info.columns["volume"]]
	adjClose := record[info.columns["adj_Close"]]
	return &finance{
		date:               date,
		open:             open,
		high:     high,
